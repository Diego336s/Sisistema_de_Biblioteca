        ...
        <div class="container">
            ...
            <div class="nav-scroller py-1 mb-3 border-bottom">
                <nav class="nav nav-underline justify-content-between">
                    ...
                    <a class="nav-item nav-link link-body-emphasis" href="listarLibroParaPrestar.jsp">
                    ...
                    <a class="nav-item nav-link link-body-emphasis" href="regresarLibro.jsp">
            ...
            <div class="row">
                ...
                <div class="col-md-6 p-md-3 mb-4 rounded text-body-emphasis bg-body-secondary">
                    ...
                    <%
                        ...
                        // Manejo de errores para el parámetro id
                        int libroId = 0;
                        try {
                        ...
                        if ("POST".equals(request.getMethod())) {
                            try {
                                ...
                                if (prestamo != null) {
                                    ...
                                    %>
                                    <div class='alert alert-danger'>No se pudo crear el préstamo. El libro no está disponible.</div>
                                    <%
                                }
                            } catch (Exception e) {
                                out.println("<div class='alert alert-danger'>Error al procesar el préstamo: " + e.getMessage() + "</div>");
                            }
                        }

                        ArrayList<Libro> libroDisponibles = administrador.getLibroDisponible();

                        if (libroDisponibles.isEmpty()) {
                    %>
                    <div class="alert alert-danger">
                        <p>No hay libros disponibles para préstamo en este momento.</p>
                    </div>
                    <p><a href="listarLibroParaPrestar.jsp" class="btn btn-primary">Volver a la lista de préstamos</a></p>
                    <% } else { %>
                    <form method="post" class="row g-3" id="formPrestamo">
                        <div class="form-group col-md-12">
                            <label for="selectLibro" class="form-label">Libro:</label>
                            <select class="form-select" id="selectLibro" name="id" required>
                                <option value="">Seleccione un libro</option>
                                <% for (Libro libro : libroDisponibles) {%>
                                <option value="<%= libro.getId()%>" 
                                        <%= (libro.getId() == libroId) ? "selected" : ""%>>
                                    <%= libro.getNombre()%> (<%= libro.getTipoLibro()%>, <%= libro.getAutor()%>)
                                </option>
                                <% } %>
                            </select>
                        </div>
                        ...
                        <div class="col-md-6">
                            <label for="prestatario" class="form-label">Nombre del Prestatario</label>
                            <input type="text" class="form-control" id="prestatario" name="prestatario" 
                                   required  
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" id="documento" name="documento" 
                                   required  title="Solo números">
                        </div>

                        <div class="col-md-6">
                            <label for="diasPrestamo" class="form-label">Días de Préstamo</label>
                            <input type="number" class="form-control" id="diasPrestamo" name="diasPrestamo" 
                                   required min="1">
                        </div>

                        <div class="col-12 centrar">
                            <button type="submit" class="btn btn-primary">Prestar</button>
                            <a href="listarLibroParaPrestar.jsp" class="btn btn-danger">Cancelar</a>
                        </div>
                    </form>
                    <% }%>
                </div>
